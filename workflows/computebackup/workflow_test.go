...
func TestNewWorkflow(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantErr: "failed to create VM backup workflow",
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			// The composed workflow must run PowerOnPBS → BackupVMs then
			// PowerOffPBS, and must NOT include the directory backup activity.
			...
			assert.NotContains(t, activities, "backup.BackupDirs")
		})
	}
}
