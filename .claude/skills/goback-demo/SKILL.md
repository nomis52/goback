...
```bash
scripts/demo-start.sh          # build + launch, prints http://localhost:8080/
scripts/demo-start.sh --run    # also trigger the safe demo workflow once up
```

The server runs in the background so the user can inspect the UI. Give them the
URL and ask them to reply OK when done. The demo workflow (started by `--run`,
or via the **Run** button on the Workflows tab) just sleeps ~2s across 3 steps
and touches no infra.
...
If `demo-start.sh` reports the port is busy, a previous demo is still running —
run `scripts/demo-stop.sh` first.
